...
class ConsolePlayer(BasePokerPlayer):
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        action, amount = self.__receive_action_from_camera(valid_actions)
    ...
    def __receive_action_from_camera(self, valid_actions):
        response = 'f' #  determine_move()
        if response in self.__gen_valid_flg(valid_actions):
            if response == 'f':
                return valid_actions[0]['action'], valid_actions[0]['amount']
            elif response == 'c':
                return valid_actions[1]['action'], valid_actions[1]['amount']
            elif response == 'r':
                # raise_amount = determine_raise_amount()
                raise_amount = 10
                return valid_actions[2]['action'], raise_amount
        else:
            return self.__receive_action_from_console(valid_actions)
